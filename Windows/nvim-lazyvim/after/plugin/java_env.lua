-- ~/.config/nvim/after/plugin/java_env.lua
local workspace_dir = vim.fn.expand("D:/Disk Utilities/Programs/Development/DK's Coding Workspace/java")
...
-- Create a simple user command: :JavaEnv
vim.api.nvim_create_user_command("JavaEnv", function()
  cd_to_workspace()
end, { nargs = 0 })

-- Optional mapping: <leader>dw to jump to workspace dir (change leader if needed)
vim.keymap.set("n", "<leader>dw", cd_to_workspace, { desc = "cd to D:/Disk Utilities/Programs/Development/DK's Coding Workspace/java" })
