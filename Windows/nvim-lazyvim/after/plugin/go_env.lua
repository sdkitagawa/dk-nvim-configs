-- ~/.config/nvim/after/plugin/go_env.lua
local workspace_dir = vim.fn.expand("D:/Disk Utilities/Programs/Development/DK's Coding Workspace/go")
...
-- Create a simple user command: :GoEnv
vim.api.nvim_create_user_command("GoEnv", function()
  cd_to_workspace()
end, { nargs = 0 })

-- Optional mapping: <leader>dw to jump to workspace dir (change leader if needed)
vim.keymap.set("n", "<leader>dw", cd_to_workspace, { desc = "cd to D:/Disk Utilities/Programs/Development/DK's Coding Workspace/go"})
