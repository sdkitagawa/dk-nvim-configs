...
-- C:/Users/user_name/AppData/Local/nvim/after/plugin/lua_env.lua

local env_path = vim.fn.stdpath("config") .. "/.env"
local env = require("env").load_env(env_path)

local workspace_dir = env.LUA_WINDOWS_WORKSPACE or vim.fn.expand(".")
...
vim.api.nvim_create_user_command("LuaEnv", cd_to_workspace, {})
vim.keymap.set("n", "<leader>dw", cd_to_workspace, { desc = "Change directory to Lua workspace" })